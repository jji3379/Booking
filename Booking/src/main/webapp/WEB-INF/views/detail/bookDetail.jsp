		...
		<c:forEach var="b" items="${bookDetail }">
			<div class="hori">
				<table class="tb">
					...
					<tr class="trCount">
						<th>수량</th>
						<td class="quantity"><input id="countP" type="number"
							name="count" class="numBox" min="1" max="100" value="1" /></td>
					</tr>
					<tr>
						<td class="trButton" colspan="2"><input id="idP"
							type="hidden" name="id" value="${id }" /> <input id="imageP"
							type="hidden" name="image" value="${b.image }" /> <input
							id="titleP" type="hidden" name="title" value="${b.title }" /> <input
							id="priceP" type="hidden" name="price" value="${b.price }" /> <input
							id="d_priceP" type="hidden" name="d_price" value="${b.discount }" />
							<input type="hidden" id="isbnP" name="isbn" value="${b.isbn }" />
							<input type="hidden" id="publisherP" name="publisher"
							value="${b.publisher }" /> <input type="hidden" id="authorP"
							name="author" value="${b.author }" />
							<button class="btn btn-outline-light" id="insertBtn"
								type="button" onclick="insert()">장바구니</button>
							<button class="btn btn-outline-light" id="directBtn"
								onclick="direct()">바로구매</button> <a
							href="${pageContext.request.contextPath }/new-review/${b.isbn }"
							class="btn btn-outline-light" id="reviewBtn"> 리뷰쓰기</a></td>
					</tr>
				</table>
				<div class="divR">
				...
				<div class="shipping">
					...
					<div class="ship-R">
						<ul class="ship-guide">
							...
							<li><p>출고가능 시간이 서로 다른 상품을 함께 주문할 경우 출고가능 시간이 가장 긴 상품을
									기준으로 배송됩니다.</p></li>
							<li><p>군부대, 교도소 등 특정기관은 우체국 택배만 배송가능합니다.</p></li>
							<li><p>배송비는 업체 배송비 정책에 따릅니다.</p></li>
							<li><p>도서 구매 시, 2만 원 이상 무료, 2만원 미만 2500 원</p></li>
					...
					<div class="review-R">
						<div class="review-box">
							<div class="head-star review-top">
								...
								<span class="total-value"></span>
								<p class="total-count">
									( 총 <span id="total-count">2</span> 건 )
								</p>
							</div>
							<ul class="sortWrap">
								<li class="review-sort"><a href="#" id="regdate"
									onclick="pagingList(0, 'regdate')">최근순</a></li>
								<li class="review-sort"><a href="#" id="rating"
									onclick="pagingList(0, 'rating')">별점순</a></li>
							</ul>
							<ul class="reviewList">
								<!-- 더미  -->
								<%-- <li>    
